defmodule SmartSort.AI_Reactor.Steps.AnalyzeUnsubscribePageStep do
  ...
  def run(arguments, _context, _options) do
    ...
    {:ok, wallaby_session} = Wallaby.start_session()
    ...
    case HttpUnsubscribeAgent.execute_http_unsubscribe(
           url,
           wallaby_session,
           extraction_result.user_email || "user@example.com"
         ) do
      {:ok, success_message} ->
      ...
      {:requires_analysis, page_data} ->
        ...
        Logger.info("[ANALYZE_STEP] HTML length: #{String.length(page_data.html_content)} chars")
